...
int left_motor_max_speed = 60000; //60000
int left_motor_acceleration = 500000;
int left_motor_pullInSpeed = 1500;
int left_motor_num_repeats = 1;
int left_motor_short_move_num_steps = 40;
int left_motor_short_move_num_repeats = 5;
...
int right_motor_num_repeats = 1;
int right_motor_short_move_num_steps = 50;
int right_motor_short_move_num_repeats = 0;
...
void Left_Stimulus()
{
  ...
  for (int i = 0; i < left_motor_num_repeats; i++) {

    left_motor.setTargetRel(left_motor_num_steps*left_motor_direction);
    controller.move(left_motor);    // Do the move

    for (int i = 0; i < left_motor_short_move_num_repeats; i++) {
      left_motor.setTargetRel(-left_motor_short_move_num_steps*left_motor_direction);
      controller.move(left_motor);    // Do the move
      left_motor.setTargetRel(left_motor_short_move_num_steps*left_motor_direction);
      controller.move(left_motor);    // Do the move
    }
...
void Right_Stimulus()
{
  ...
  for (int i = 0; i < right_motor_num_repeats; i++) {
    
    right_motor.setTargetRel(right_motor_num_steps*right_motor_direction);
    controller.move(right_motor);    // Do the move

    for (int i = 0; i < right_motor_short_move_num_repeats; i++) {
      right_motor.setTargetRel(-right_motor_short_move_num_steps*right_motor_direction);
      controller.move(right_motor);    // Do the move
      right_motor.setTargetRel(right_motor_short_move_num_steps*right_motor_direction);
      controller.move(right_motor);    // Do the move
    }
